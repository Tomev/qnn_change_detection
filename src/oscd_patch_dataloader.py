...
class ChangeDetectionDataset(Dataset):
    ...
    def __init__(
        ...
        self.weights = [
            self.fp_modifier * true_pix / n_pix,
            (n_pix - true_pix) / n_pix,
        ]  # What is this?
        self.class_weights = [1 / true_pix, 1 / (n_pix - true_pix)]
        # self.class_weights = [ self.fp_modifier * 2 * true_pix / n_pix, 2 * (n_pix - true_pix) / n_pix]
    ...
    def get_sample_weights(self):
        ...
        for current_patch in self.patch_coords:
            ...
            label = torch.where(
                sum(label) / (self.patch_side * self.patch_side) < 0.5, 0, 1
            )
            change_count += label
            patch_count += 1
        ...
        # print(patch_count, change_count.item())
        # sys.exit(-1)
        change_count = change_count.item()
        self.sample_weights = [
            1 / (patch_count - change_count * 0.9),
            1 / (change_count * 0.9),
        ]
    ...
    def __getitem__(self, idx):
        ...
        # print("patch", limits[0],limits[1] ,limits[2],limits[3] )
        # print("center",centre[0],centre[1])
        ...
        # label = self.change_maps[im_name][centre[0],centre[1]]
