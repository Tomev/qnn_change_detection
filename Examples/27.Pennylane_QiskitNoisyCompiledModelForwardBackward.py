...
def main() -> None:
    ...
    print("\tCreate noisy circuit.")
    noisy_circuit = qml.add_noise(ideal_circuit, pl_model)

    print("\n\n\tCircuit execution:")
    print(f"\t\tIdeal: {ideal_circuit(data, weights)}")
    print(f"\t\tIdeal: {noisy_circuit(data, weights)}")
    ...
    weights = weights[1]  # TODO TR: Probably can be done smarter.
    weights, _ = opt.step_and_cost(noisy_circuit, data, weights)
    print(f"\tFinal weights:\n\t\t{weights}")
