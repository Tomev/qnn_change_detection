# We now know how to access all the error data we need to replicate the noise model from
# a selected qiskit backend just as they do it in qiskit.aer (see example 15).
#
# What we want to do now is prepare pennylane.NoiseModel using that data. In the next
# example we will try to apply a noise model created this way.
...
def get_readout_errors_for_experiment(backend, qubit_map):
...
def get_gate_errors_for_experiment(backend, qubit_map):
...
def get_noise_model(gate_errors, name_op_map):
    # https://docs.pennylane.ai/en/stable/code/qml_noise.html

    # We first prepare the dict

    noise_model_map = {}

    def generate_error_channel(ge):
        def error_channel(op, **metadata):
            for q in op.wires:
                if ge[3]:  # Depolarizing error defined
                    qml.DepolarizingChannel(ge[3]["depol_param"], q)
                if ge[2]:  # Relaxation error defined
    ...
    for ge in gate_errors:
        ...
        if ge[0] == "reset":
            continue
        c = qml.noise.op_eq(name_op_map[ge[0]]) & qml.noise.wires_in(list(ge[1]))
        noise_model_map[c] = generate_error_channel(ge)

    return NoiseModel(noise_model_map)
...
def main() -> None:
    ...
    print("\n\tGet gate errors!\n")
    gate_errors = get_gate_errors_for_experiment(backend, qubit_map)

    # Noise model is actually created only from gate_errors. Readout error is just adding
    # BitFlips at the end of the target circuit.

    # print("\tGet readout errors!\n")
    # readout_errors = get_readout_errors_for_experiment(backend, qubit_map)

    # Manually prepare the name -> operation map for brisbane
    print("\n\tPrepare noise model!\n")
    name_op_map = {
        "ecr": qml.ECR,
        "x": qml.X,
    ...
    noise_model = get_noise_model(gate_errors, name_op_map)
